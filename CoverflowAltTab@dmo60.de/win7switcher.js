		...
		show: function() {			
			...
			for (i in this._windows) {
				...
				if (compositor) {
					...
					clone.target_x = Math.round(monitor.width * 0.4);
					clone.target_y = Math.round(monitor.height * 0.5) - OFFSET; 
...
//			}
		...
		_updateCoverflow: function(direction, loop) {
			...
			for (i in this._previews) {
				...
				i = parseInt(i);
				distance = (this._currentIndex > i) ? this._previews.length - this._currentIndex + i
				...
				if (distance == this._previews.length - 1 && direction == "next") {
					...
					Tweener.addTween(preview, {
						opacity: 0,
						x: preview.target_x - Math.sqrt(this._previews.length) * 200 ,
						y: preview.target_y - Math.sqrt(this._previews.length) * 100,
						width: Math.max(preview.target_width * ((20 - 2 * this._previews.length) / 20), 0),
						height: Math.max(preview.target_height * ((20 - 2 * this._previews.length) / 20), 0),
						time: animation_time / 2,
						transition: transition_type,
						onCompleteParams: [preview, distance, animation_time, transition_type],
						onComplete: this._onFadeComplete,
					...
					Tweener.addTween(preview, {
						opacity: 255,
						x: preview.target_x - Math.sqrt(distance) * 200,
		...
		_onFadeComplete: function(preview, distance, animation_time, transition_type) {
			if (distance == 0)
				preview.raise_top();
			else
				preview.lower_bottom();
			
			preview.x = preview.target_x - Math.sqrt(distance) * 200;
		...
		_keyPressEvent: function(actor, event) {
			...
			} else if (keysym == Clutter.Right || keysym == Clutter.Down) {
				this._next();
			} else if (keysym == Clutter.Left || keysym == Clutter.Up) {
				this._previous();
			// D -> Show desktop
